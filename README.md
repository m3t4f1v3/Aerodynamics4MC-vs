...
`/aero_client_l2 status` 会显示实际 `brickSize`、准备预算、step/publish 耗时和 native brick runtime 状态。`AERO_LBM_CLIENT_L2_MAX_ACTIVE_BRICKS`、`AERO_LBM_CLIENT_L2_PUBLISH_INTERVAL_TICKS`、`AERO_LBM_CLIENT_L2_STATIC_BUILD_CELLS_PER_TICK`、`AERO_LBM_CLIENT_L2_COARSE_SEED_CELLS_PER_TICK` 可用于控制测试压力；默认游戏行为仍是 32³。
